...
| [`Uart_tx`](../lib/staging/uart_tx.ml) | One clockless 10-bit lane transaction per 8N1 frame: start 0, byte LSB first, stop 1. Each bit is `2 * half_period_i` clocks. Idle is driven high while enabled; reset/disable/abort release the pin. | Independent bit-center receiver test, back-to-back start, and disable test. [`p2_uart_tb.v`](../tinytapeout/test/p2_uart_tb.v) covers emitted Verilog when HDL tools are available. |
| [`Uart_slice`](../lib/staging/uart_slice.ml) | P2.7's first working slice. A six-state harness claims the transmit pin for the engine, spends one bit period of [`Timing`](../lib/pluggable_primitives/timing.ml) countdown driving the resting level, offers the byte to `Uart_tx`, commits every bit through a masked [`Pin_bank`](../lib/pluggable_primitives/pin_bank.ml) engine write, and releases the claim after the stop bit. The pin therefore lags the lane by exactly one cycle, uniformly. Reset, disable, and abort release pin and claim together. | One independent receiver decodes four producers to the same frame; [`uart_frame.trace`](../test/integration/uart_slice/uart_frame.trace) is the saved trace. [`p2_uart_slice_tb.v`](../tinytapeout/test/p2_uart_slice_tb.v) repeats the receiver in Verilog and diffs the same file. |
...
afterwards -- [`uart_slice.ml`](../lib/staging/uart_slice.ml) and everything under
...
hardware in [`uart_slice.ml`](../lib/staging/uart_slice.ml), and one independent receiver
...
arbitration in both the protocol-specific [`uart_slice.ml`](../lib/staging/uart_slice.ml) and the
generic [`observed_transfer_bank.ml`](../lib/staging/observed_transfer_bank.ml), each for one engine.
The loaded [`Integrated_core`](../lib/emulator_core/integrated_core.ml) now supplies the central core/lane
