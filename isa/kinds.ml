...
(* Module: "kinds.ml" *)
(* The small direction-neutral enumerations shared by the instruction specification, the
   ...
   They live in THIS library, below the model, because an instruction field names one of
   them: [Wait_edge] encodes an [Edge.t] and [Fifo_push] encodes a [Fifo_id.t] and a
   [Blocking.t], and the value an encoded field carries is the constructor's position in
   [all]. An encoder and a decoder that disagreed about that order would disagree about
   what a program means, so the order is declared once, here, and both read it (P1.5).
*)
