...
module Make (D : Device) = struct
  ...
  let run ?(context = 6) ?(transcript = false) config ~scenario ~edge_budget =
    ...
    let model = ref (D.Model.create config) in
    let dut_monitor = ref (D.Monitor.create config) in
    let model_monitor = ref (D.Monitor.create config) in
    ...
    let rec advance edge items =
      ...
      | item :: rest ->
        ...
        then
          ...
          let model_pre = D.Model.pre_edge !model item in
          ...
          | None ->
            ...
            D.Dut.edge dut;
            model := D.Model.edge !model item;
            let dut_post = D.Dut.post_edge dut in
            let model_post = D.Model.post_edge !model in
            let next_dut_monitor, dut_items =
              D.Monitor.observe !dut_monitor ~edge dut_post
            in
            let next_model_monitor, model_items =
              D.Monitor.observe !model_monitor ~edge model_post
            in
            dut_monitor := next_dut_monitor;
            model_monitor := next_model_monitor;
            push
              { Edge_record.edge
              ; item = item_sexp
              ; pre_edge = dut_pre
            ...
            (match
             ...
             | None ->
               ...
               then advance (edge + 1) rest
                 ...
                 mismatch
                   Phase.Monitor
                   (Detail.Items { expected = model_items; actual = dut_items })))
    in
    advance 0 scenario
  ;;
  ...
  module Failure = struct
    ...
    let to_lines ?(redact_source = false) t =
      List.concat
        [ [ [%string "%{D.name}: model/Hardcaml comparison failed"]; "" ]
        ; [ "reproduction:" ]
