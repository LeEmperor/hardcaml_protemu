...
(* The P2.1 pin bank described once for the shared harness.
   ...
   holds which pins - comes from [Protemu_model.Pin_bank], which has no Hardcaml
   ...
   Those are properties of this block's port contract, so [Model] below states them once,
   from the contract. It calls nothing in [lib/].
...
module Reference = Protemu_model.Pin_bank
...
module Reject_code = struct
  ...
  let of_reject (reject : Protemu_model.Fault.Reject.t) =
    match reject with
...
(* The independent reference. [bank] is [Protemu_model.Pin_bank]; the three fields beside
   it are the port contract this environment owns. *)
module Model = struct
  type t =
    { config : Config.t
    ; bank : Reference.t
